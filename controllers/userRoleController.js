...
class UserRoleController {
    async create(req,res){
        ...
        const item = await UserRole.create({value})
        return res.json(item)
    }

    async getAll(req,res){
        const items = await UserRole.findAll()
        return res.json(items)
    }

    async getOne(req,res){
        const { id } = req.params
        const item = await UserRole.findOne({ where: { id } })
        res.json(item)
    }
    ...
    async editOne(req,res,next){
        ...
        const oldItem = await UserRole.findOne({ where: { id } })
        if (!oldItem)
            return next(ApiError.badRequest("Отсутствуют данные для редактирования!"))
        const newItem = await UserRole.update({ value }, { where: { id } })
